    ...
    private void fillInfo(Text [] lines)
    {
        for(int i=0;i<3;i++)
        {
            ...
            if(i==0)
            {
                ...
                Data[2]=lines[i].getValue().substring(5,14);//Br. dokumenta
            ...
            else if(i==1)
            {
                ...
                int checkY;
                String yy = lines[i].getValue().substring(0, 2);
                checkY = Integer.parseInt(yy);
                String mm = lines[i].getValue().substring(2, 4);
                String dd = lines[i].getValue().substring(4, 6);
                if (checkY > 10) {
                ...
                do
                {
                    Data[4]=lines[i].getValue().substring(7,8);//Spol
                    if(Data[4].equals(" ") || Data[4].matches("[0-9]+"))
                    {
                        if(temp)
                        {
                            Data[4]=lines[i].getValue().substring(8,9);//Spol
                        }else{
                            Data[4]=lines[i].getValue().substring(6,7);//Spol
                        }
                        temp=!temp;
                ...
                Data[5]=getCountryName(lines[i].getValue().substring(15,18));//Puno ime nacionalnosti
            }
            else{
                int n=0;
                int m=0;
